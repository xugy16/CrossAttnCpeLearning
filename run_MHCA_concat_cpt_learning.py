...
if __name__ == '__main__':
    ...
    # --------------------
    config = get_config()
    wandb.init(#project="concat_combine_cpt_learn",
               project="",
               entity="xugy007",
               config=config)
    config = wandb.config
    ...
    net = ConcatCptNet(config, glove_obj, train_ds)
    net.cuda()
    ...
    with torch.no_grad():
        net.eval()
        # SlowValidate(evaluator, net, test_dl)
        FastValidate(config, evaluator, net, test_dl)
    """
    ...
    for epoch in range(start_epoch, config.max_epochs):
        ...
        print("Current Learning Rate is  {}".format(warmup_adam.lr_base))
